...
// Key is versioned with the PanProfile layout (v2 +stainless, v3 +burner
// map): an old-layout blob under an old key would misparse by size, so it is
// simply orphaned (re-learn the pans).
uint32_t storage_get_profiles(void* out, uint32_t maxBytes) {
  ensure();
  return (uint32_t)s_prefs.getBytes("profs3", out, maxBytes);
}
void storage_set_profiles(const void* data, uint32_t bytes) {
  ensure();
  if (bytes == 0) s_prefs.remove("profs3");
  else s_prefs.putBytes("profs3", data, bytes);
}
